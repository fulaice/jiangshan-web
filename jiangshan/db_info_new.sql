...
CREATE TABLE `provinces` (
   ...
   PRIMARY KEY (`id`),
   UNIQUE KEY (`provinceid`)
) ENGINE=InnoDB AUTO_INCREMENT=40 DEFAULT CHARSET=utf8mb4;

-- create table
CREATE TABLE `cities` (
   `id` int(11) NOT NULL AUTO_INCREMENT,
   `cityid` int(11) NOT NULL COMMENT '城市编码',
   `city` varchar(40) NOT NULL COMMENT '城市名称',
   `provinceid` int(11) NOT NULL COMMENT '所属省份编码',
   PRIMARY KEY (`id`),
...
CREATE TABLE `countries` (
   `id` int(11) NOT NULL AUTO_INCREMENT,
   `countryid` int(11) NOT NULL COMMENT '区县编码',
   `country` varchar(40) NOT NULL COMMENT '区县名称',
   `cityid` int(11) NOT NULL COMMENT '所属城市编码',
   PRIMARY KEY (`id`),
